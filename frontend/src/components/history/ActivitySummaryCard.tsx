...
import type { ActivitySummary } from '../../mocks/historyMockData';
...
export const ActivitySummaryCard: React.FC<Props> = ({ summary }) => {
  const rows = [
    ...
    { label: 'Yesterday', value: summary.yesterday, accent: 'bg-zinc-400', valueColor: 'text-zinc-700' },
  ...
  return (
    <div className="bg-white shadow-sm rounded-xl p-6 hover:shadow-lg hover:-translate-y-[2px] transition-all duration-300 border border-gray-300">
      ...
      <div className="flex items-center gap-3 mb-5">
        ...
        <h3 className="text-lg font-bold text-zinc-900 tracking-tight">Summary</h3>
      </div>
        ...
        {rows.map((row) => (
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm font-medium text-zinc-500 group-hover:text-zinc-800 transition-colors duration-200">
                {row.label}
              </span>
            </div>
