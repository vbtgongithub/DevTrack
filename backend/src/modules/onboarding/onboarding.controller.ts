...
export const onboardingController = {
  /**
   * GET /api/onboarding/progress - Get onboarding progress for current user
   */
  async getProgress(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (!analytics) {
        // Initialize onboarding analytics document
      ...
      // Map DB funnelSteps to frontend SseOnboardingSteps
  ...
  /**
   * POST /api/onboarding/step/:stepId/complete - Complete specific onboarding step
   */
  async completeStep(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (!userId || !stepId) {
        res.status(400).json({ error: 'User ID and Step ID are required' });
        return;
      }
      ...
      logger.error('[onboarding] Failed to complete step', { error });
      res.status(500).json({ error: 'Failed to complete step' });
    }
  },

  /**
   * POST /api/onboarding/step/:stepId/skip - Skip specific onboarding step
   */
  async skipStep(req: Request, res: Response): Promise<void> {
    try {
      ...
      logger.error('[onboarding] Failed to skip step', { error });
      res.status(500).json({ error: 'Failed to skip step' });
    }
  },

  /**
   * POST /api/onboarding/complete - Complete overall onboarding
   */
  async completeOnboarding(req: Request, res: Response): Promise<void> {
    try {
      ...
      // Mark all ONBOARDING_STEPS completed or skipped
